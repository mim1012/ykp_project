...
class DashboardController extends Controller
{
    ...
    public function salesTrend(Request $request): JsonResponse
    {
        try {
            ...
            // 단일 쿼리로 일별 집계 (DB별 DATE() 지원)
            $rows = (clone $baseQuery)
                ->select([
                    DB::raw('DATE(sale_date) as d'),
                    DB::raw('COUNT(*) as activations'),
                    DB::raw('COALESCE(SUM(settlement_amount),0) as sales'),
                ])
            ...
            for ($i = 0; $i < $days; $i++) {
                ...
                $trendData[] = [
                    'date' => $date,
                    'day_label' => date('j일', strtotime($date)),
                    'sales' => $row->sales ?? 0,
            ...
            return response()->json(['success' => false, 'error' => 'Failed to load trend'], 500);
        }
    }
    ...
    public function dealerPerformance(Request $request): JsonResponse
    {
        ...
        // 통신사별 분석 (PostgreSQL 호환: 퍼센트는 PHP에서 계산)
        $baseMonthly = Sale::whereYear('sale_date', substr($yearMonth, 0, 4))
            ->whereMonth('sale_date', substr($yearMonth, 5, 2));

        $totalMonthlyCount = (clone $baseMonthly)->count();

        $carrierStats = (clone $baseMonthly)
            ->select([
                'carrier',
            ...
            ->groupBy('carrier')
            ->get()
            ->map(function ($row) use ($totalMonthlyCount) {
                $row->percentage = $totalMonthlyCount > 0
                    ? round(($row->count * 100.0) / $totalMonthlyCount, 1)
                    : 0.0;
                return $row;
            });
